...
import DoctorsManagements from "@/components/admin/DoctorsManagements";
import Navbar from "@/components/Navbar";
import { useGetAppointments } from "@/hooks/use-appointmet";
...
const AdminDashboardClient = () => {
  ...
  const stats = {
    ...
    activeDoctores: doctors.filter((doc) => doc.isActive).length,
    completedAppointments: appointments.filter(
      (appoint) => appoint.status === "COMPLETED"
    ).length,
  };

  if (doctorsLoading || appointmentsLoading) return <LoadingUI></LoadingUI>;
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 pt-24">
        ...
        <AdminStats
          ...
          activeDoctors={stats.activeDoctores}
          completedAppointments={stats.completedAppointments}
        ></AdminStats>

        <DoctorsManagements></DoctorsManagements>
      </div>
    </div>
  );
};
